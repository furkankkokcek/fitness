...
function showWeeklySummary(w){
  ...
  modal.innerHTML=`
    <div class="ms">
      <div class="mt2">HAFTA ${w+1} TAMAMLANDI!</div>
      <div class="msub">Haftanın özeti</div>
      <div style="margin:16px 0">${daysHtml}</div>
      <div style="display:flex;justify-content:space-between;align-items:center;background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:12px 16px;margin-bottom:${prList.length?'12px':'16px'}">
      ...
      ${prList.length?`<div style="background:rgba(232,255,71,.08);border:1px solid rgba(232,255,71,.2);border-radius:6px;padding:10px 14px;margin-bottom:16px;font-size:12px;line-height:1.8">${prList.join('<br>')}</div>`:''}
...
function openWeeklyReport(){
  ...
  const dayRows=days.map((d,i)=>`
    <div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border)">
      <span style="font-family:var(--fd);font-size:14px;color:var(--muted);width:16px;text-align:center">${dayIcons[i]}</span>
      <div style="flex:1">
  ...
  const prHtml=prs.length?`
    <div style="background:rgba(232,255,71,.07);border:1px solid rgba(232,255,71,.2);border-radius:6px;padding:12px 14px;margin-bottom:14px">
      <div style="font-size:11px;color:var(--accent);font-weight:700;margin-bottom:8px">Bu Haftanın PR'ları</div>
  ...
  // Kilo bölümü
  const wtHtml=curWt?`
    <div style="display:flex;align-items:center;justify-content:space-between;background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:12px 16px;margin-bottom:14px">
      <div>
        <div style="font-size:11px;color:var(--muted);font-weight:600">Vücut Kilosu</div>
      ...
      ${wtDiff!==null?`<div style="text-align:right">
        <div style="font-size:11px;color:var(--muted);font-weight:600">Değişim</div>
  ...
  modal.innerHTML=`
    <div class="ms" style="padding:20px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:16px">
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:12px 8px;text-align:left">
          <div style="font-family:var(--fd);font-size:26px;color:${completedDays===3?'var(--accent)':'var(--text)'}">${completedDays}/3</div>
          <div style="font-size:10px;color:var(--muted);margin-top:3px">Gün</div>
        </div>
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:12px 8px;text-align:left">
          <div style="font-family:var(--fd);font-size:${totalMin>99?'20':'26'}px;color:var(--text)">${totalTimeStr}</div>
          <div style="font-size:10px;color:var(--muted);margin-top:3px">Süre</div>
        </div>
        <div style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:12px 8px;text-align:left">
          <div style="font-family:var(--fd);font-size:${streak>99?'20':'26'}px;color:var(--success)">${streak}</div>
          <div style="font-size:10px;color:var(--muted);margin-top:3px">Seri</div>
        </div>
      </div>
      ...
      <!-- Toplam hacim -->
      <div style="display:flex;justify-content:space-between;align-items:center;background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:12px 16px;margin-bottom:14px">
        <span style="font-size:13px;font-weight:600;color:var(--muted)">Toplam Hacim</span>
